...
def dwgc():
    ...
    # report.write("用地许可证号:%s<br>" % ydxkzh)
    ...
    # report.write("工程预算:%s<br>" % gcys)
    # report.write("工程决算:%s<br>" % gcjs)
    # report.write("高度:%s<br>" % gd)
    # report.write("建筑面积:%s<br>" % jzmj)
    # report.write("用地面积:%s<br>" % ydmj)
    # report.write("地上层数:%s<br>" % dscs)
    # report.write("地下层数:%s<br>" % dxcs)
    # report.write("基础类型:%s<br>" % jclx)
    # report.write("结构类型:%s<br>" % jglx)
    report.write(
        "<hr><br>")
    fo.flush()
    report.flush()
                ...
                if ',' in line:
                    ...
                    try:
                        if int(ab) > 0:
                            ...
                            hs=end-begin
                            hs=str(int((end-begin).total_seconds()))
                            # print('OCR第%s页——第%s页,共%s页'%(str(qsy),str(zzy),str(int(zzy)-int(qsy)+1)))
                            # os.system('espeak -vzh 正在识别%s第%s页至第%s页,共%s页'%(a[0],str(qsy),str(zzy),str(int(zzy)-int(qsy)+1)))
                            # print(colorama.Fore.LIGHTGREEN_EX +'PDF切割完成，用时%s秒,平均每页耗时%s秒\n'%(hs,str(int(hs)//int(ab))))
                            print(colorama.Fore.LIGHTGREEN_EX +'PDF正常\n')
                            # os.system('espeak -vzh 识别完成，用时%s秒,平均每页耗时%s秒'%(hs,str(int(hs)//int(ab))))
                            shutil.move(wstream.name, xmmc + '/' + xmmc+'/')
                            isok = '正常'
                            # print ("pdf done")
                    except BaseException:
                        # print ("%s.pdf error!\n"%str(count))
                        print(colorama.Fore.LIGHTRED_EX +'%sPDF错误\n'%(a[0]))
                        # os.system('espeak -vzh %sPDF文件错误'%(a[0]))
                        isok='错误'
                        # pass
...
if __name__ == "__main__":
    ...
    # print (table)
    print('完成!\n')
    # os.system('espeak -vzh 完成，收工！')
    # webbrowser.open(xmmc+'/'+xmmc+'.xml',0,False)
    # webbrowser.open(xmmc+'/report.html',0,False)
